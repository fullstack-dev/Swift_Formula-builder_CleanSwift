...
- (void)onTag: (UIButton *)btn {
    SKTag *tag = _tags[[self.subviews indexOfObject: btn]];
    SKTagButton *skBtn = (SKTagButton *)btn;
    skBtn.isTapped = !skBtn.isTapped;
    [skBtn setBackgroundImage:skBtn.isTapped ? [SKTagButton imageWithColor:tag.bgColor] : tag.bgImg
...
- (void)addTag: (SKTag *)tag {
    ...
    if (tag.selected) {
        btn.isTapped = tag.selected;
        [btn setBackgroundImage:btn.isTapped ? [SKTagButton imageWithColor:tag.bgColor] : tag.bgImg
                         forState:UIControlStateNormal];
    }
